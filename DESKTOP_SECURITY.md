...
The app retains Electron's sandbox and context isolation with Node integration disabled. Navigation keeps the main window on the application page. Web addresses open in the default browser; executable schemes and addresses containing embedded credentials are rejected. Embedded navigation is limited to script-free GM and player document previews, which have their own sandboxes and restrictive content policies. Webviews and device, camera, microphone, location, notification, and clipboard-read permission requests are denied.

The GM run-sheet frame permits same-origin access so the trusted application can handle local section jumps and keyboard return. Its sandbox still disables document scripts, and its content policy blocks scripts and external resources. Campaign text is escaped before rendering. Player document previews retain their opaque-origin sandbox. The GM download uses the captured document; changed preparation, source material, workspace context, or player mode invalidates that preview. Browser checks exercise these controls, including attempted script execution and a resource request rejected by the content policy. The distinction between sandbox origin and script permissions follows the [iframe sandbox documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/iframe#sandbox).
