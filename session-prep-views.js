...
function prepPinnedRecordDetails(record) {
  ...
  return `<details class="prep-existing-body"><summary>Saved details for this pinned record</summary>${content || `<p>No additional details are saved for this record.</p>`}</details>`;
}
...
function prepSceneMarkup(scene, index, total) {
...
function sessionPrepView(campaign) {
  ...
  return `<div class="session-prep-page" data-prep-id="${esc(prep.id)}">
    ...
    <div class="prep-workspace-grid"><div class="prep-outline"><section class="card prep-panel"><div class="section-title"><div><p class="eyebrow">FLEXIBLE RUN OF PLAY</p><h2>Scene outline</h2></div><span class="tag">${prep.scenes.length} scenes</span></div><p class="prep-help">Prepare situations and meaningful choices. Reorder the outline as your session takes shape.</p><div class="prep-scene-list">${prep.scenes.length ? prep.scenes.map((scene, index) => prepSceneMarkup(scene, index, prep.scenes.length)).join("") : `<div class="prep-empty prep-empty-scene"><strong>Give the table its first situation.</strong><p>Add a scene with a clear pressure or opportunity, the details you need, and a question the players can answer.</p></div>`}</div><button class="secondary-button prep-add-button" type="button" data-prep-add="scenes">＋ Add scene</button></section>
      ...
      <section class="card prep-panel"><div class="section-title"><div><p class="eyebrow">CLUES TO PUT IN PLAY</p><h2>Secrets & revelations</h2></div></div><div class="prep-item-list">${prep.revelations.length ? prep.revelations.map(item => `<div class="prep-text-row" data-prep-card="${esc(item.id)}"><label class="prep-grow">Revelation<textarea ${prepField("revelations", item.id, "text")} rows="2" maxlength="1000" placeholder="What useful truth might the characters discover?">${esc(item.text || "")}</textarea></label>${prepRemove("revelations", item.id, "revelation")}</div>`).join("") : `<p class="prep-empty">Write discoveries you can reveal through conversation, exploration, or consequences.</p>`}</div><button class="secondary-button prep-add-button" type="button" data-prep-add="revelations">＋ Add revelation</button></section>
      <section class="card prep-panel"><div class="section-title"><div><p class="eyebrow">WHAT ADVANCES</p><h2>Clocks & counters</h2></div></div><p class="prep-help">Track faction plans, approaching danger, or progress. Set the starting value for play.</p><div class="prep-item-list">${prep.clocks.length ? prep.clocks.map(item => `<article class="prep-clock" data-prep-card="${esc(item.id)}"><div class="prep-inline-heading"><label>Clock or counter<input ${prepField("clocks", item.id, "label")} maxlength="160" value="${esc(item.label || "")}" placeholder="The watch closes in" /></label>${prepRemove("clocks", item.id, "clock")}</div><div class="prep-clock-values"><label>Starting value<input ${prepField("clocks", item.id, "value")} type="number" min="0" max="${esc(item.max)}" value="${esc(item.value)}" /></label><label>Segments / maximum<input ${prepField("clocks", item.id, "max")} type="number" min="1" max="20" value="${esc(item.max)}" /></label></div></article>`).join("") : `<p class="prep-empty">Add a pressure that moves when time passes or the characters act.</p>`}</div><button class="secondary-button prep-add-button" type="button" data-prep-add="clocks">＋ Add clock</button></section>
      <section class="card prep-panel"><div class="section-title"><div><p class="eyebrow">BEFORE THE TABLE</p><h2>Prep tasks</h2></div></div><div class="prep-item-list">${prep.tasks.length ? prep.tasks.map(item => `<div class="prep-task ${item.done ? "is-done" : ""}" data-prep-card="${esc(item.id)}"><input ${prepField("tasks", item.id, "done")} type="checkbox" ${item.done ? "checked" : ""} aria-label="Mark prep task complete" /><label class="prep-grow"><span class="sr-only">Prep task</span><input ${prepField("tasks", item.id, "text")} maxlength="500" value="${esc(item.text || "")}" placeholder="Prepare a map, read a rule, choose music…" /></label>${prepRemove("tasks", item.id, "task")}</div>`).join("") : `<p class="prep-empty">Keep the remaining work here: maps, handouts, rules checks, and table logistics.</p>`}</div><button class="secondary-button prep-add-button" type="button" data-prep-add="tasks">＋ Add task</button></section></aside></div>
  </div>`;
}
